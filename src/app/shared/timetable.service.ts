...
export class TimetableService {
  ...
  getSingleStationTimetable(
    criteria: SingleDirectionTimetable
  ): Observable<StationTimeTable[]> {
    return this.http.get<StationTimeTable[]>(this.serviceBaseUrl + "/single", {
      params: {
        ...
        toStation:
          criteria.destStationId === undefined
            ? ""
            : criteria.destStationId.toString(),
        live: criteria.isLive ? "1" : "0",
        diagramType: criteria.diagramType,
        diagramSpecified: criteria.diagramSpecified,
        direction:
          criteria.direction === undefined ? "" : criteria.direction.toString(),
        travelTime: criteria.startTime.replace(":", "") + "00",
      },
    });
  }

  getSingleTrainTimetable(
    criteria: StationTimeTable
  ): Observable<StationTimeTable[]> {
  ...
  getStationTimetable(
    criteria: StationTimetableCriteria
  ): Observable<InterchangeGuideAndTimetable[]> {
    return this.http.get<InterchangeGuideAndTimetable[]>(
      this.serviceBaseUrl + "/station",
      {
        params: {
