...
export class SingleDirectionTimetableSectionComponent implements OnInit {
  ...
  refreshLive(component) {
    ...
    that.timetableService.getSingleStationTimetable(that.timetable).subscribe(
      (data) => {
        if (that.trains) {
        ...
        if (data.length > 0) {
          const nextRefreshSeconds =
            that.getNextRefreshSeconds(data[0].fromDeparture) * 1000;
  ...
  refresh(component) {
    ...
    component.trains.forEach((element) => {
      element.countdown = component.getCountDownDisplay(
        element.fromArrival,
        currentTime
      );
    });
  ...
  getCurrentTimeToNumber() {
    ...
    return (
      hours * 3600 + currentTime.getMinutes() * 60 + currentTime.getSeconds()
    );
  }

  getCountDownDisplay(targetTime: string, currentTime: Date) {
    const countdown = 0;
    const targetTimeSeperate = targetTime.split(":");
    const targetSeconds = parseInt(targetTimeSeperate[2]);
    const targetMinutes = parseInt(targetTimeSeperate[1]);
    const targetHours = parseInt(targetTimeSeperate[0]);
    ...
    let deltaMinutes =
      (targetHours - currentHours) * 60 + targetMinutes - currentMinutes;
    if (deltaMinutes >= 60) {
      return ">60:00";
    }
  ...
  getNextRefreshSeconds(targetTime: number) {
    if (this.timetable.isLive) {
      ...
      const currentHours = currentTime.getHours();
      return (
        (targetHours - currentHours) * 3600 +
        (targetMinutes - currentMinutes) * 60 +
  ...
  trackStation(item: StationTimeTable) {
    this.dialog.open(StationTimetableForInterchangeComponent, {
      ...
      data: {
        ...
        diagramType:
          this.timetable.diagramType === undefined
            ? Constants.CurrentDiagramType(new Date().getDay())
            : this.timetable.diagramType,
